...
export default async function HomePage() {
  ...
  return (
    <div className="grid min-h-screen w-full grid-rows-[1fr_20px] bg-red-200/50 dark:bg-dark-surface items-start justify-items-center gap-16 overflow-x-hidden font-(family-name:--font-geist-sans) sm:p-12">
      <main className="row-start-1 flex w-full max-w-5/6 flex-col items-center gap-4 pt-6 sm:items-start sm:px-8 z-10">
        <h1 className="flex w-full items-center text-center text-2xl">
          <div className="border-login-hover dark:border-login h-px w-full border" />
          <div
            className={`px-2 text-xl font-bold text-nowrap sm:text-3xl ${biancha.className} text-login-hover dark:text-login`}
          >
            The Blogging Photographer
          </div>
          <div className="border-login-hover dark:border-login h-px w-full border" />
        </h1>
        <div className="z-10 text-login-hover dark:text-login text-4xl">
        ...
        <Break className="border-login-hover dark:border-login w-full" />
        ...
        <div className="grid w-full grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {featuredPosts?.map((blog, ind) => (
            <BlogCardServer
              ...
              className="border-none shadow-none bg-transparent hover:backdrop-blur dark:hover:shadow-dark-accent rounded-md"
            />
          ))}
        </div>
      </main>
